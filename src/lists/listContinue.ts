...
import { continuationMarker, findSameIndentParentMarker } from "./listMarker";
...
export async function handleListContinue(): Promise<void> {
  ...
  const parent = findSameIndentParentMarker(document, selection.active.line);
  if (parent) {
    await insertBelow(selection, `${parent.indent}${continuationMarker(parent.parentMarker)}`);
    return;
  }
