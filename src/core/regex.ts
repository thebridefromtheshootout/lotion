...
export const Regex = {
  ...
  dbRequiredLine: /^\s+required:\s*(true|false)\s*$/,
  dbUniqueLine: /^\s+unique:\s*(true|false)\s*$/,
  dbColumnDefaultLine: /^\s+default:\s*(.+)$/,
  dbMinLine: /^\s+min:\s*(-?\d+(?:\.\d+)?)\s*$/,
  dbMaxLine: /^\s+max:\s*(-?\d+(?:\.\d+)?)\s*$/,
  dbViewsLine: /^views:\s*$/,
